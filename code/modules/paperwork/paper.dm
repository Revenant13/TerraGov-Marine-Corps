...
/obj/item/paper/tutorial/mechanics
	...
	<BR>
	Weapon damage has three attributes: Damage, penetration. The way damage is calculated is by taking said value and multiplying it by the amount of armor the xenomorph has, IE a xeno with 40 bullet armor will receive only 60% of the advertised damage. Penetration is the amount of armor that will be reduced from the xeno before the aforementioned calculation. You can test all guns and spawn xenomorphs to try them on if you are a ghost/observer, by going to the Ghost tab and clicking "Join Valhalla".<BR>
	<BR>
